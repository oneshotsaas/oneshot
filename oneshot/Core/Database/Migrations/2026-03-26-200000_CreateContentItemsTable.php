<?php

namespace OneShot\Core\Database\Migrations;

use CodeIgniter\Database\Migration;

class CreateContentItemsTable extends Migration
{
    public function up(): void
    {
        $this->forge->addField([
            'id'                   => ['type' => 'INT', 'unsigned' => true, 'auto_increment' => true],
            'type'                 => ['type' => 'VARCHAR', 'constraint' => 10, 'default' => 'post'],
            'title'                => ['type' => 'VARCHAR', 'constraint' => 255],
            'slug'                 => ['type' => 'VARCHAR', 'constraint' => 191],
            'canonical_category_id'=> ['type' => 'INT', 'unsigned' => true, 'null' => true, 'default' => null],
            'image'                => ['type' => 'VARCHAR', 'constraint' => 512, 'null' => true, 'default' => null],
            'meta_title'           => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => true, 'default' => null],
            'meta_description'     => ['type' => 'VARCHAR', 'constraint' => 512, 'null' => true, 'default' => null],
            'content'              => ['type' => 'TEXT', 'null' => true, 'default' => null],
            'template'             => ['type' => 'VARCHAR', 'constraint' => 50, 'default' => 'post'],
            'is_active'            => ['type' => 'TINYINT', 'constraint' => 1, 'default' => 1],
            'created_at'           => ['type' => 'DATETIME', 'null' => true],
            'updated_at'           => ['type' => 'DATETIME', 'null' => true],
            'deleted_at'           => ['type' => 'DATETIME', 'null' => true],
        ]);
        $this->forge->addPrimaryKey('id');
        $this->forge->addUniqueKey('slug');
        $this->forge->addKey('type');
        $this->forge->addKey('is_active');
        $this->forge->createTable('content_items');
    }

    public function down(): void
    {
        $this->forge->dropTable('content_items');
    }
}
