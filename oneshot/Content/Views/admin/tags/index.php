    ...
    <table class="table table-sm w-full">
        ...
        <tbody class="divide-y divide-base-200">
            ...
            <tr class="hover:bg-base-200/50 transition-colors">
                <td>
                    <a href="<?= route_to('admin.content.tags.edit', signId($tag->id)) ?>" class="font-medium hover:opacity-70 transition-opacity">
                        <?= esc($tag->title) ?>
                    </a>
                </td>
                <td class="font-mono text-xs">
                    <a href="<?= content_url($tag, 'tag') ?>" target="_blank" rel="noopener" class="opacity-60 hover:opacity-100 transition-opacity"><?= esc($tag->slug) ?></a>
                </td>
                <td><span class="badge badge-sm <?= $tag->is_active ? 'badge-success' : 'badge-ghost' ?>"><?= $tag->is_active ? __('content.active', 'Active') : __('content.inactive', 'Inactive') ?></span></td>
                <td>
                    <div class="flex items-center justify-end gap-1">
                        <div class="tooltip tooltip-left" data-tip="<?= __('content.view', 'View') ?>">
                            <a href="<?= content_url($tag, 'tag') ?>" target="_blank" rel="noopener" class="btn btn-ghost btn-xs btn-square">
                        ...
                        <form method="post" action="<?= route_to('admin.content.tags.delete', signId($tag->id)) ?>" onsubmit="return confirm('<?= __('content.delete_confirm', 'Delete?') ?>')">
                            <?= csrf_field() ?>
            ...
            <?php endforeach ?>
            <?php if (empty($tags)): ?>
            <tr><td colspan="4" class="py-12 text-center text-sm opacity-40"><?= __('content.no_tags', 'No tags yet') ?></td></tr>
            <?php endif ?>
        </tbody>
    </table>
</div>
